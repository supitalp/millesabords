"""Tests for the Animals card (monkeys and parrots count as the same symbol)."""
import pytest
...
def test_merge_increases_combo():
    # 3 monkeys + 3 parrots: default=100+100=200; animals=6-combo=1000
    h = held((Face.MONKEY, 3), (Face.PARROT, 3), (Face.SWORD, 1))
    assert score(1, h, DEFAULT_CONFIG) == 200
    assert score(1, h, ANIMALS) == 1000
...
def test_8_animals_combo():
    ...
    # animals: COMBO_SCORE[8]=4000, full board +500
    assert score(0, h, ANIMALS) == 4500
...
def test_below_3_animals_no_combo():
    ...
    # animals: animals=2→0, swords=3→100, coin=2→0+200
    assert score(1, h, ANIMALS) == 300
...
def test_full_board_bonus_mixed_animals():
    ...
    assert score(0, h, ANIMALS) == 1100  # 500 + 100 + 500
...
# ---------------------------------------------------------------------------
# DP: animals solution is correctly solved
# ---------------------------------------------------------------------------

def test_dp_solves_animals():
    sol = get_solution(ANIMALS)
    assert len(sol.states) > 0
    assert (sol.V >= 0).all()


def test_v_at_least_stop_value_animals():
    sol = get_solution(ANIMALS)
    assert (sol.V >= sol.stop_values).all()


def test_merged_animals_higher_stop_value():
    """Same hand scores more with animals than without, when animals form a larger combo."""
    # 3 monkeys + 3 parrots + 2 swords, n_skulls=0
    h = held((Face.MONKEY, 3), (Face.PARROT, 3), (Face.SWORD, 2))
    # animals: animals=6→1000, swords=2→0. Full board: sword=2 < 3 → no bonus. Total=1000
    assert score(0, h, ANIMALS) == 1000
