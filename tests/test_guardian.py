"""Tests for the Guardian card (one-time skull reroll ability)."""
import pytest
...
from solver.actions import guardian_kept_options, valid_actions
from solver.stats import compute_stats
from solver.report import turn_ev

GUARDIAN = CARD_CONFIGS["guardian"]
...
def test_guardian_state_space_doubles():
    sol_base = get_solution(DEFAULT_CONFIG)
    sol_guardian = get_solution(GUARDIAN)
    assert len(sol_guardian.states) == 2 * len(sol_base.states)


# ---------------------------------------------------------------------------
# Actions: Guardian options available iff n_skulls >= 1 and not used
# ---------------------------------------------------------------------------

def test_guardian_options_available_with_skull():
    # n_skulls=1, skull_reroll_used=False → Guardian options exist
...
def test_guardian_reroll_count():
    # Keep all held, reroll only the freed skull → n_reroll = 1
    state = State(1, held((Face.SWORD, 7)), skull_reroll_used=False)
    full_hold = state.held
    s = compute_stats(state, full_hold, GUARDIAN, use_guardian=True)
    assert s.n_reroll == 1  # just the freed skull


def test_guardian_reroll_plus_normal():
    # Keep 5 swords, reroll 2 normal + 1 skull = 3 dice
    state = State(1, held((Face.SWORD, 7)), skull_reroll_used=False)
    kept = held((Face.SWORD, 5))
    s = compute_stats(state, kept, GUARDIAN, use_guardian=True)
    assert s.n_reroll == 3


# ---------------------------------------------------------------------------
# DP: Guardian value > base game (one extra reroll opportunity)
# ---------------------------------------------------------------------------

def test_guardian_ev_higher_than_base():
    base_ev = turn_ev(DEFAULT_CONFIG)
    guardian_ev = turn_ev(GUARDIAN)
    assert guardian_ev > base_ev


def test_v_at_least_stop_value():
    sol = get_solution(GUARDIAN)
    assert (sol.V >= sol.stop_values - 1e-9).all()


def test_guardian_not_used_state_has_higher_v():
    """State with Guardian available (not used) should have V ≥ same state with it used."""
    sol = get_solution(GUARDIAN)
    # Pick a state with 1 skull where Guardian could help
    h = held((Face.SWORD, 7))
...
# ---------------------------------------------------------------------------
# Stats: using Guardian sets skull_reroll_used=True in next states
# ---------------------------------------------------------------------------

def test_guardian_action_transitions_to_used_states():
    """After using Guardian, next states should all have skull_reroll_used=True."""
    sol = get_solution(GUARDIAN)
    state = State(1, held((Face.SWORD, 7)), skull_reroll_used=False)
...
def test_guardian_with_2_skulls_reduces_bust_risk():
    """With 2 skulls and unused Guardian, reactive rescue lowers bust risk vs base game."""
    ...
    s_base = compute_stats(state, kept, DEFAULT_CONFIG, use_guardian=False)
    # Guardian (reactive): exactly-3-skull outcomes get a rescue roll → ≈7.4%
    s_guardian = compute_stats(state, kept, GUARDIAN, use_guardian=False)
    assert s_guardian.p_lose < s_base.p_lose
